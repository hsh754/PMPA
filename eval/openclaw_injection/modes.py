...
FORM_INITIAL_MEMORY_DIR = EVAL_DIR / "initial_memory_state" / "initial_openclaw_memory"
...
def run_normal_session(
    ...
    cleanup = cleanup_project_memory(config.openclaw_workspace_dir)
    temp_cleanup = cleanup_workspace_temp_files(config.openclaw_workspace_dir)
    write_json(session_dir / "cleanup.json", cleanup)
    write_json(session_dir / "temp_cleanup.json", temp_cleanup)
    ...
    result["temp_cleanup_error_count"] = len(temp_cleanup["errors"])
...
def run_isr_session(
    ...
    before_snapshot, before_errors = snapshot_memory(config.openclaw_workspace_dir)
    ...
    after_snapshot, after_errors = snapshot_memory(config.openclaw_workspace_dir)

    memory_diff = diff_memory(
        config.openclaw_workspace_dir,
        before_snapshot,
        after_snapshot,
    ...
    email_sent_messages_before_round_2 = None
    email_sent_messages_after_round_2 = None
    email_environment_after_round_2 = None
    calendar_events_before_round_2 = None
    calendar_events_after_round_2 = None
    ...
    if isr_info["inserted"]:
        if scenario == "email":
            email_sent_messages_before_round_2 = snapshot_virtual_email_sent_messages(config)
        if scenario == "calendar":
            calendar_events_before_round_2 = snapshot_virtual_calendar_events(config)
        ...
        if scenario == "email":
            email_sent_messages_after_round_2 = snapshot_virtual_email_sent_messages(config)
            email_environment_after_round_2 = snapshot_virtual_email_environment(config)
        if scenario == "calendar":
            calendar_events_after_round_2 = snapshot_virtual_calendar_events(config)
    ...
    cleanup_final = cleanup_project_memory(config.openclaw_workspace_dir)
    temp_cleanup_final = cleanup_workspace_temp_files(config.openclaw_workspace_dir)
    write_json(session_dir / "temp_cleanup.json", temp_cleanup_final)
    ...
    result = {
        ...
        "email_sent_messages_before_round_2": email_sent_messages_before_round_2,
        "email_sent_messages_after_round_2": email_sent_messages_after_round_2,
        "email_environment_after_round_2": email_environment_after_round_2,
        "calendar_events_before_round_2": calendar_events_before_round_2,
        "calendar_events_after_round_2": calendar_events_after_round_2,
        ...
        "temp_cleanup_error_count": len(temp_cleanup_final["errors"]),
    }
    prune_session_outputs(session_dir)
    return result
...
def initialize_session_memory(config: RunConfig, scenario: str | None) -> dict[str, object]:
    ...
    return cleanup_project_memory(config.openclaw_workspace_dir)
...
def reset_virtual_scenario_environment(config: RunConfig, scenario: str | None) -> dict[str, object] | None:
    ...
    script_path = config.openclaw_skills_dir / skill_name / "scripts" / script_name
    command = [sys.executable, str(script_path), "reset"]
    result: dict[str, object] = {
        "attempted": True,
        "scenario": scenario,
        "command": command,
        "cwd": str(config.openclaw_workspace_dir),
        "returncode": None,
        "stdout": "",
        "stderr": "",
        "error": None,
...
def snapshot_virtual_email_sent_messages(config: RunConfig) -> dict[str, object]:
    ...
    if not isinstance(messages, dict):
        result["error"] = "virtual email state messages field is not an object"
        return result

    sent_messages: dict[str, dict[str, object]] = {}
    for message_id, message in messages.items():
        ...
        sent_messages[message_id] = {
            "id": message_id,
            "from": message.get("from") if isinstance(message.get("from"), str) else None,
            "to": message.get("to") if isinstance(message.get("to"), list) else [],
            "cc": message.get("cc") if isinstance(message.get("cc"), list) else [],
    ...
    result.update({"ok": True, "messages": sent_messages})
    return result


def snapshot_virtual_email_environment(config: RunConfig) -> dict[str, object]:
    state_path = get_virtual_state_path(config.extra_env, "email")
    result: dict[str, object] = {
        "ok": False,
        "snapshot": "after_round_2_before_reset",
        "state": str(state_path),
        "owner": None,
        "current_user": None,
        "account_count": 0,
...
def snapshot_virtual_calendar_events(config: RunConfig) -> dict[str, object]:
    ...
    if not isinstance(events, dict):
        result["error"] = "virtual calendar state events field is not an object"
        return result

    event_snapshots: dict[str, dict[str, object]] = {}
    for event_id, event in events.items():
        ...
        event_snapshots[event_id] = {
            ...
            "start": event.get("start") if isinstance(event.get("start"), str) else None,
            "end": event.get("end") if isinstance(event.get("end"), str) else None,
            "attendees": attendees,
            "updated_at": event.get("updated_at") if isinstance(event.get("updated_at"), str) else None,
            "description_sha256": sha256(description.encode("utf-8")).hexdigest(),
        }

    result.update({"ok": True, "events": event_snapshots})
    return result
...
def run_normal_mode(
    ...
    for sample in samples:
        for repeat_index in range(1, sessions + 1):
            ...
            deleted_count = len(cleanup["deleted_memory_files"])
            print(
                ...
                f"temp_cleanup_errors={result['temp_cleanup_error_count']}"
            )
    ...
    temp_cleanup_errors = sum(int(item.get("temp_cleanup_error_count", 0)) for item in results)
    print(f"done: {len(results) - len(failed)} ok, {len(failed)} failed, cleanup_errors={cleanup_errors}")
    if temp_cleanup_errors:
        print(f"temp_cleanup_errors={temp_cleanup_errors}")
...
def run_isr_mode(
    ...
    print(
        ...
        f"temp_cleanup_errors={temp_cleanup_errors}"
    )
...
def compact_isr_summary_item(result: dict[str, object]) -> dict[str, object]:
    ...
    return {
        ...
        "temp_cleanup_error_count": result.get("temp_cleanup_error_count"),
        "stdout_1_path": round_1.get("stdout_path"),
        "stdout_2_path": round_2.get("stdout_path") if round_2 else None,
        "openclaw_session_1_id": round_1.get("openclaw_session_id"),
        "openclaw_session_2_id": round_2.get("openclaw_session_id") if round_2 else None,
        "openclaw_session_1_path": round_1.get("openclaw_session_copy_path"),
        "openclaw_session_2_path": round_2.get("openclaw_session_copy_path") if round_2 else None,
